...
const FALLBACK_IMAGE_URL = "https://images.pexels.com/photos/15822340/pexels-photo-15822340/free-photo-of-a-woman-in-a-traditional-indian-sari.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2";

const Hero: React.FC = () => {
  const { settings } = useSettings();

  const imageUrl = settings.heroImageUrl || FALLBACK_IMAGE_URL;
  const tagline = settings.heroTagline || "Style, Redefined.";
  const subtitle = settings.heroSubtitle || "Discover curated collections of apparel, jewelry, and accessories for the modern woman.";

  return (
    <section 
      className="relative h-screen flex items-center justify-center text-center text-text-primary overflow-hidden" 
      role="banner"
      aria-label="Homepage hero section"
    >
      <div 
        className="absolute inset-0 bg-cover bg-center hero-animated-bg"
        style={{ backgroundImage: `url('${imageUrl}')` }}
        aria-hidden="true"
      ></div>
      <div className="absolute inset-0 bg-background-start/30 dark:bg-black/50"></div>
